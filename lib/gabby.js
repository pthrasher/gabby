...
function Gabby(path, options) {
  ...
  options.engine = options.engine || 'html';
...
gabby.build = function(callback) {
  var self = this;
  var build = function() {
    self.log.started('Building static files');
    fs.exists(self.path+'/public', function(exists) {
      ...
      if (exists) {
        rat.copy(self.path+'/public', self.path+'/generated', {limit: 512}, function(errors) {
...
gabby.createHTMLPageFromData = function(data, callback) {
  ...
  if (data.template) {
    ...
    this.render(data.template, data, function(html) {
      ...
      if (self.options.layout === true) {
        writeContent();
      }
      else {
        self.render(self.options.layout, data, function(_html) {
          html = _html;
          writeContent();
        });
      }
...
/**
 * Render Underscore `template` file with given `data` and pass HTML result to
 * callback.
 *
 * @param String template
 * @param Mixed data
...
/**
 * Render Swig `template` file with given `data` and pass HTML result to
 * callback.
 *
 * @param String template
 * @param Mixed data
 ...
 * @api private
 */
var swigInstance = null;
gabby.render.swig = function(template, data, callback) {
  var self = this,
      getSwig = function(){
        if (swigInstance != null) return swigInstance;
        swigInstance = require('swig');
        swigInstance.init({
          root: self.path + "/templates",
          cache: false
        });
        return swigInstance;
      };
