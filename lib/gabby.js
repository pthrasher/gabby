...
function Gabby(path, options) {
  ...
  Object.defineProperty(this, 'layout', {
    enumerable: true,
    writable: true,
    value: __dirname + '/../src/templates/layout.html'
  });
  ...
  self.generateHTML(function() {
    self.generateCSS(function() {
      self.generateJS(function() {
        self.log.finished('Building static files');
      });
    });
  });
};
    ...
    files.forEach(function(file) {
      self.transcompile(file, function(css) {
        ...
        if (files.length === cssCollection.length) {
          css = uglifycss.processString(cssCollection.join('\n'));
          fs.writeFile(
    ...
    files.forEach(function(file) {
      self.transcompile(file, function(js) {
        ...
        if (jsCollection.length === files.length) {
          var result = uglifyjs.minify(jsCollection.join(';'));
          fs.writeFile(
...
gabby.createHTMLPageFromData = function(data, callback) {
  ...
  if (data.template) {
    ...
    this.render(data.template, data, function(html) {
      data.content = html;
      self.render(self.layout, data, function(html) {
        fs.writeFile(
          ...
          function(err) {
            if (err) throw err;
            self.log.debug(
              'Created ' + self.path + '/generated' + data.url + '.html'
            );
            callback.call(self);
          }
        );
    ...
    this.render(self.layout, data, function(html) {
      fs.writeFile(
        self.path + '/generated' + data.url + '.html',
        html,
        'utf8',
...
gabby.render.html = function(template, data, callback) {
  ...
  fs.readFile(template, 'utf8', function(err, html) {
    if (err) throw err;
    var html = require('plates').bind(html, data);
